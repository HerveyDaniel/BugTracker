...
/* put axios post request to register user credentials in the server. 
Put logic to check if password and confirm password states are the same.*/
const signUp = () => {
...
export const Login = () => {
    ...
    /* implement these in the login method (refer to vid). If user
    is prompted to login after trying to access a certain page,
    these will allow the user to be sent to the page they  wanted
    to go to after a successful login/authentication. */
    const navigate = useNavigate();
    const location = useLocation();
    const from = location.state?.from?.pathname || "/home";
    const demofrom = location.state?.from?.pathname || "/demohome";

    /* put axios post request in this function to send username and password */
